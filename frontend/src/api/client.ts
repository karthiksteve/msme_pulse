...
export interface XAILoanRequest {
  msme_id: string
  asking_amount: number
  loan_purpose?: string
}

export const fetchLoanExplanation = (payload: XAILoanRequest) =>
  api.post('/api/v1/xai/loan-explanation', payload).then(r => r.data)

export const fetchLoanExplanationGet = (msmeId: string, askingAmount: number, loanPurpose?: string) =>
  api.get(`/api/v1/xai/loan-explanation/${msmeId}`, {
    params: { asking_amount: askingAmount, loan_purpose: loanPurpose }
  }).then(r => r.data)
